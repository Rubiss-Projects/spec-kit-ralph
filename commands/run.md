...
6. **Verify startup, then confirm and exit**:
   ...
   - Startup evidence must come from the orchestrator: its `Ralph Loop - {feature}` header with an iteration line for the selected feature. An already-complete, valid feature exits before that header and prints `All tasks are already complete!` followed by a standalone `<promise>COMPLETE</promise>` line; accept those fresh lines as immediate completion evidence and report that the feature was already complete. This path does **not** print `Ralph Loop Summary`. If a run instead finishes during verification with a summary, inspect and report that observed outcome. Check the exit result when available; a nonzero exit, completion-blocked error, or failure summary must be reported as failure. Do not wait for an agent/model response as startup evidence.
